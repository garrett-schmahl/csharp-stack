...
class SinglyLinkedList {
  ...
  //could be better if we found the last value and set its .next to head
  betterConcat(addList) {
    if (this.isEmpty()) {
      return addList;
    }
    if (addList.head === null) {
  ...
  moveMinToFront() {
    let runner = this.head;
    let listMinData = runner.data;
    while (runner.next) {
      if (runner.data < listMinData) {
        listMinData = runner.data;
      }
    }
    removeVal(listMinData);
    insertAtFront(listMinData);
    return this;
  }
}
