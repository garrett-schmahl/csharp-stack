...
class Queue {
  ...
  isSumOfHalvesEqual() {
    if (this.isEmpty()) {
      ...
      console.log("One item queue");
      return undefined;
    } else if (this.size % 2 != 0) {
      console.log("Warning: Odd length queue.");
    }
    let stopPoint = this.size() / 2; //truncated .5 if shown
    let i = 0;
    let itemSum1 = 0;
    let itemSum2 = 0;
    while (i <= stopPoint) {
      itemSum1 += this.items[i];
      i++;
    }
    while (i < this.size()) {
      itemSum2 += this.items[i];
      i++;
    }
    if (itemSum1 == itemSum2) {
      return true;
    }
    return false;
  }
